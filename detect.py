...
def detect(FrameQueue: queue.Queue):
    weights='./best.pt' # model path or triton URL
    source='0' # file/dir/URL/glob/screen/0(webcam)
    data='data/coco128.yaml' # dataset.yaml path
    imgsz=(640640) # inference size (heightwidth)
    conf_thres=0.25 # confidence threshold
    iou_thres=0.45 # NMS IOU threshold
    max_det=1000 # maximum detections per image
    device='' # cuda devicei.e. 0 or 0,1,2,3 or cpu
    view_img=False # show results
    save_txt=False # save results to *.txt
    save_conf=False # save confidences in --save-txt labels
    save_crop=False # save cropped prediction boxes
    nosave=True # do not save images/videos
    classes=None # filter by class: --class 0or --class 0 2 3
    ...
    exist_ok=False # existing project/name okdo not increment
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in det:
                    if int(cls) == 0:  # Check if the class is 0, a 'turret'
                        FrameQueue.put(det[:, :4])
